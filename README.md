...
| :--- |:------:|:------------------------------------------------------------------------------------|
| **Пользовательская списочная коллекция** |   +    | Класс `BookCollection`. Использует `UserList` вместо `[]`. |
| **Пользовательская словарная коллекция** |   +    | Класс `IndexDict`. Использует `UserDict` вместо `{}`. Поддерживает индексы по ISBN, Автору и Году.                     |
| **Отказ от Raw Types** |   +    | Методы поиска и срезы возвращают **объекты коллекций**, а не списки. |
...
### 2. Коллекции
*   **`BookCollection`**:
    *   Обертка над `collections.UserList`.
    *   Отвечает за хранение и порядок добавления.
    *   Методы: `add_book`, `remove_book`.
    *   Доступ: по индексу и срезам (`__getitem__`).
*   **`IndexDict`**:
    *   Обертка над `collections.UserDict`.
    *   Хранит три внутренних индекса (`isbn`, `authors`, `years`), которые также реализованы через `UserDict`.
    *   **Особенность:** Списки книг внутри индексов хранятся как экземпляры `BookCollection`.
    *   Методы поиска (`search_by_author`) возвращают коллекцию, а не список.
